  ...
  try {
    if (browser.family === 'firefox') {
      if (browser.majorVersion) {
        ...
        // right now we are gating on firefox version 134 to turn on BiDi.
        // this is subject to change, especially when resourceType then becomes supported
        return majorVersion >= 134
      }
    }
  } catch (e) {
    return false
